...
**EXTENDED Phase 33 Unit 33.3** — Q76 architectural candidate **RESOLVED via Option A** per [ADR-0023](./0023-per-user-account-subscriptions.md) D-A: `subscriber.userId text references users(id) on delete cascade` nullable column extension via migration `0008_subscriber_user_id.sql`. **Closes the Phase-31+ anticipation at 2-phase Q-carryover** (Phase 30 → 31 → 32 → 33). Anonymous email-only path preserved verbatim (NULL `userId` = Phase-30 path; NOT NULL = Phase-33 authenticated subscriber). Subscribe-route POST handler reads `safeAuth()` for session + auto-populates `userId` per ADR-0023 D-C; existing rows update `userId` per `decideSubscriberUserId` pure helper per ADR-0023 D-E/D-F/D-G semantics (anonymous → authenticated migration on first signed-in re-submit + conservative-preserve on cross-user re-subscribe). **Per-problem subscriptions** + **cross-domain summary subscriptions** remain Phase 34+ Q76-sibling-expansion candidates per ADR-0023 D-H. **First ADR to close an explicit prior ADR's Phase-N+1 anticipation at tight follow-on** in project history; mirrors ADR-0020's pattern of explicit ADR-0012 D-B partial-supersession but for anticipation-vs-supersession. **Subscriber-list-email arc completes Phase 30 → 31 → 32 → 33 = first 4-phase complete-feature pair in project history** (foundation + scheduler + cleanup + authenticated-linkage). **Fifth APPEND-pattern ADR D-clause cluster** in project history (ADR-0013 D-F = 2 APPENDs; ADR-0018 D-G = 3 APPENDs; ADR-0021 D-F = 1 APPEND Phase 31; ADR-0022 D-D = 1 APPEND Phase 32; ADR-0021 D-C = 1 APPEND Phase 33 = the fifth) — confirms the APPEND-not-EDIT pattern as the recurring shape for inheritance-pattern carry-forward.
...
**EXTENDED Phase 31 Unit 31.4** — Weekly **digest email template** realized as `lib/email/templates/digest.tsx` per [ADR-0022](./0022-weekly-digest-scheduler.md) D-E (Unit 31.2; closes Phase-30 B.15 item 1 at 1-phase carryover — tightest follow-on cadence mirrors Phase 21/23/25/27/29). Template count Phase 30 → 31 = 2 → **3** (verification + welcome + digest); React Email adoption threshold (~5) NOT crossed; React Email deferral remains. **Pattern correction carried forward**: Phase-30 in-flight discovery that `react-dom/server` is blocked in Next.js App Router was retroactively codified as "plain template-literal string builder" precedent for verification + welcome; **digest template ships native to this pattern** (no TSX → JSX → renderToStaticMarkup pipeline; the file extension stays `.tsx` for editor consistency but only exports a pure function returning an HTML string). **`DigestPayload` type-level contract reused verbatim** from Phase-5 `lib/digest/build-digest.ts` — first time the same payload shape is consumed by two render targets (RSS XML Phase 5 + email HTML Phase 31). FR translation + email i18n keys deferral preserved unchanged at Phase 31 close (Phase 32+ curator-track per ADR-0022 D-H). **Third APPEND-pattern ADR D-clause cluster shipped** in project history (ADR-0013 D-F = 2 APPENDs Phase 11 + Phase 30; ADR-0018 D-G = 3 APPENDs; ADR-0021 D-F = 1 APPEND Phase 31; **first ADR D-clause to receive a Phase-N+1 APPEND at 1-phase age** — prior APPENDs spanned 5+ phases between source and APPEND).
