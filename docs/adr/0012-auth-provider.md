...
**SUPERSEDED (partial) Phase 28 Unit 28.4** — D-B's "GitHub OAuth as the ONLY initial provider" + "multi-provider expansion forbidden in Phase 9" clauses are **lifted by [ADR-0020](./0020-multi-provider-oauth.md)** (Phase 28). Phase-9 Class B item 8 (multi-provider OAuth expansion) carried **17 phases** as the strongest non-multi-phase architectural patience signal in project history; Phase 28 closed it. ADR-0020 D-B pins Google as the second provider via `next-auth/providers/google`; provider count is now exactly **2** at Phase 28 close (GitHub first, Google second; ordering preserved per Phase-9 user expectation). Adding a third provider (GitLab / email-link / passkeys / etc.) requires an ADR-0020 amendment or a new ADR per ADR-0020 D-B's "no third provider without ADR" rule (which mirrors this clause's spirit at the new boundary). ADR-0020 D-D **preserves D-E's curator-of-record model verbatim**: `users.githubLogin` is populated only via the GitHub `linkAccount` event (Phase-9 Unit 9.6 verbatim); non-GitHub users (Google sign-ins) leave `githubLogin` NULL and cannot be named as `editorial.primary_curator` on rating-action YAML files. Widening curator eligibility to non-GitHub users is [Q74](../../OPEN_QUESTIONS.md#q74-non-github-users-as-curators) (architectural; Phase 29+ if curator demand surfaces). All other ADR-0012 D-clauses (D-A runtime singleton; D-C database-backed sessions; D-D redirect-to-provider sign-in UX; D-E user identity model) **stay in force unchanged**. ADR-0020 D-F extends D-D's redirect-to-provider pattern to per-provider buttons (mirrors verbatim per provider; no chooser modal). This is the **first time an ADR D-clause is partially superseded by a follow-on ADR** rather than amended via an inline implementation note — establishing the convention for future inter-ADR supersession-without-full-replacement (vs ADR-0005 immutability for code-shipping ADRs; this ADR + ADR-0020 stand side-by-side with explicit cross-references). The `Supersedes/Superseded by` header line stays at "—" because the supersession is **partial**: D-A/C/D/E remain authoritative, only D-B's exclusivity claim is lifted.
