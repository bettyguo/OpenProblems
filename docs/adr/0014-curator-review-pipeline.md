...
- **CLI helper** `pnpm emit-challenge-action <id>` reading the challenge from the DB + scaffolding the YAML at `content/problems/<slug>/ratings/<date>-<slug>.yaml`. Mirrors Phase-5's `extract-leaderboard` shape. Eliminates the "write the YAML by hand" friction while preserving the manual commit step. **Possible Q59 candidate.** **REALIZED Phase 22 Unit 22.1** — `scripts/emit-challenge-action.ts` ships the CLI shell + `lib/rating-challenges/scaffold.ts` ships the pure-function YAML scaffolder (+11 vitest tests). Phase-22 lean: scaffold writes to gitignored `drafts/ratings/<challenge-id>.yaml` (drafts-first per `extract-leaderboard` precedent; preserves the manual commit step verbatim); PROPOSED dimension populated from challenge data; OTHER 4 dimensions placeholder + TODO rationale; `prior_action` + `signals_considered` are TODOs. Curator edits + moves to `content/` + commits + returns to UI to attach `acceptedActionId`. Phase 22 did NOT need a new ADR — the implementation realizes this D-clause without architectural surface. Phase 23+ enhancement candidates (auto-fill prior-action's 4 unchanged dimensions; `signals_considered` auto-gather; methodology-version dynamic read; batch-emit-multiple) tracked as Class B Phase-22 follow-ons. **ENHANCED Phase 23 Unit 23.1** — closed Phase-22 B.6 items 1+2 (the two largest curator-edit-friction reductions): new `readPriorRatingAction({ contentRoot, problemSlug })` helper finds + parses the most-recent prior rating-action YAML via `fs/promises.readdir` + lexical-sort-descending + per-file try/catch + `RatingActionSchema.parse`. New optional `priorAction?: PriorActionInfo | null` parameter on `buildRatingActionYaml`: when provided, the 4 OTHER dimensions copy from prior verbatim, `prior_action` field auto-sets to `<problemSlug>/<filename>`, `signals_considered` seeds from prior + placeholder TODO, `watchlist` inherits. Phase-22 null-fallback placeholder behavior preserved as regression guard. +10 vitest tests via in-test tempdir fixtures (first in-test tempdir pattern in project history). Header comment block switches to "COPIED FROM PRIOR" messaging when prior found. Phase 23 ships auto-fill always-on; `--no-prior-fill` opt-out flag tracked as Class B Phase-23 follow-on. Phase-24+ remaining enhancement candidates: methodology-version dynamic read; batch-emit-multiple.
