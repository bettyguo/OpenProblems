...
**EXTENDED Phase 33 Unit 33.3** — `subscriber.userId text references users(id) on delete cascade` nullable FK column extension via migration `0008_subscriber_user_id.sql` per [ADR-0023](./0023-per-user-account-subscriptions.md) D-A/D-B. **Third migration on the `subscriber` table within 4 phases** (Phase 30 create + 11 columns; Phase 31 `+lastDigestSentAt`; Phase 33 `+userId`) — **most active table-extension cluster in project history**. **Third migration in 4 phases** overall (Phase 30 + 31 + 33; Phase 32 gap = **first "migration-cluster-resume-after-1-phase-gap"** in project history; mirrors Phase 15-16 cluster shape with post-cluster-pause extension). USER-STATE-only invariant unchanged — `subscriber.userId` is USER-STATE (subscription tied to user identity); FK reference to `users.id` mirrors Phase-9 `accounts.userId` + `sessions.userId` + Phase-11 `ratingChallenges.userId` cascade-FK precedent (text type for UUID; cascade-on-delete). **Third APPEND note on ADR-0013** (Phase 11 + Phase 30 + Phase 33; first D-F clause with **3 APPEND notes** — ties ADR-0018 D-G as joint-most-extended ADR D-clause in project history at 3 APPENDs). Cascade-on-user-delete per ADR-0023 D-D ensures future account-deletion flow (Phase 34+ candidate) automatically removes associated subscription rows without manual cleanup. Subscribe-route POST handler reads `safeAuth()` for session per ADR-0023 D-C; `lib/subscribers/decideSubscriberUserId` pure helper realizes the D-E/D-F/D-G decision logic (anonymous → authenticated migration on signed-in re-submit + conservative-preserve on cross-user re-subscribe). **First FK column added to a project-owned table via ALTER** in project history (prior FK columns were created at table-introduction time in `0000_initial_auth` + `0001_watchlist` + `0002_rating_challenges` + `0003_rating_challenge_review`; Phase 33 = first FK column added via ALTER to an already-shipped table). Subscriber-list-email arc completes Phase 30 → 31 → 32 → 33 = **first 4-phase complete-feature pair in project history** (foundation + scheduler + cleanup + authenticated-linkage); USER-STATE-only DB scope invariant preserved across the full 4-phase arc.
