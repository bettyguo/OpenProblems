...
interface PageProps {
  params: Promise<{ locale: string; domain: string; subdomain: string }>;
}

export function generateStaticParams() {
  return locales.flatMap((locale) =>
    allDomainSubdomainPairs().map((pair) => ({ locale, ...pair })),
  );
}

export default async function SubdomainPage({ params }: PageProps) {
  const { locale, domain: domainId, subdomain: subdomainId } = await params;
