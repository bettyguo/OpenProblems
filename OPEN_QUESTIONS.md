...
- **Q68 candidate** (content moderation on bio text; ADR-0016 D-B deferral) — ~~needs moderation API integration ADR (OpenAI moderation / Perspective / custom regex+wordlist). Phase 16+ if abuse signals accumulate.~~ **RESOLVED architecturally Phase 35 Unit 35.1 → 35.3** via [ADR-0024](./docs/adr/0024-content-moderation.md) framework-only with `NoopModerator` default. `lib/users/index.ts` `updateProfile()` wires the moderation call between bio validation and DB UPDATE per ADR-0024 D-D bio surface. Concrete provider commitment (OpenAI moderation / Perspective / regex-wordlist / custom) deferred Phase 36+ per ADR-0024 D-G operational-API-gate; persistent decision audit log + appeal mechanism + bulk-rescan + policy versioning all Phase 36+ per ADR-0024 D-H.
...
- **Q68 expansion** (content moderation on uploaded images; ADR-0017 D-H deferral) — ~~extends Phase-15's Q68 candidate scope from bio text to cover uploaded imagery. Phase 17+ if abuse signals accumulate; needs moderation API integration ADR.~~ **RESOLVED architecturally Phase 35 Unit 35.1 → 35.3** via [ADR-0024](./docs/adr/0024-content-moderation.md) framework-only with `NoopModerator` default. `lib/users/index.ts` `updateProfileImage()` wires the moderation call AFTER MIME / size / magic-byte validation + BEFORE `putAvatar` Vercel Blob upload per ADR-0024 D-D avatar surface. Concrete provider commitment + audit log + bulk-rescan all Phase 36+ per ADR-0024 D-H.
...
- **Q68 expansion content moderation** on uploaded images — ~~Phase-21+ ~3-4 units; ADR-0020+ candidate; inserts moderation API call between `.rotate()` and `.toBuffer()` per ADR-0019 D-F inheritance contract.~~ **RESOLVED architecturally Phase 35 Unit 35.1 → 35.3** via [ADR-0024](./docs/adr/0024-content-moderation.md) framework-only with `NoopModerator` default + extension-point integration into bio + avatar + rating-challenge + subscribe surfaces (Unit 35.2). **Path correction**: moderation call sits in `lib/users/index.ts` `updateProfileImage()` BEFORE `putAvatar` rather than inside the sharp pipeline between `.rotate()` and `.toBuffer()` (rationale documented in ADR-0019 D-F Phase-35 APPEND). Future Phase-36+ pixel-data-post-rotation provider can refactor. **Phase-20 backfill pattern reusable** if Phase-36+ provider commits — same `scripts/backfill-content-moderation.ts` shape can retroactively scan existing avatars for moderation policy violations.
...
- **Q72 candidate (FRAMEWORK REALIZED Phase 37 Unit 37.1-2)** — Phase-37 ships the `MarkdownExtensionRegistry` framework in `lib/markdown/extensions/` (5 files; Phase-35 `lib/moderation/` shape verbatim) integrated into all 4 markdown helpers per [ADR-0018](./docs/adr/0018-markdown-sanitization.md) D-G APPEND. The framework REPLACES the "needs surface-specific schema authoring + parity-test removal + audit comment" mechanism originally described here: per-surface schema overrides now ship as `MarkdownExtensionSet.schemaOverrides` diffs via the registry, with per-surface `remarkPlugins` / `rehypePlugins` for parse-time + post-sanitize extensions. The `reviewNotesSchema` divergence path (and the three sibling-divergence paths for `bioSchema` + `rationaleSchema` + `actionRationaleSchema`) now requires only (a) authoring a new registry implementation with non-empty `schemaOverrides` for the target surface and (b) wiring the new registry into `getExtensionRegistry()` via a future Phase-38+ env-var dispatch arm — no `lib/markdown/index.ts` edit needed. Default `DefaultExtensionRegistry` keeps Phase-18/27/29 parity (zero behavioral change Day 1; 50+ existing markdown tests pass verbatim). **18-phase Q72-surfaced-to-framework-realized carryover** (Phase 18 → Phase 37). Phase-37+ concrete extensions (tables per-reviewNotes; footnotes per-rationale; wikilinks per-bio + per-actionRationale; tel-URLs per-actionRationale; etc.) deferred per [ADR-0018](./docs/adr/0018-markdown-sanitization.md) D-G APPEND F-list — each is an independent 1-2-unit Phase 38+ commit consuming the Phase-37 framework. **Original Phase-19+ Q72 candidate sub-bullet remains historically accurate** for the Phase-18-as-shipped state described in the surrounding Q71 implementation notes; this APPEND documents the framework-only-pattern resolution Phase-37 chose over a per-surface-schema-divergence resolution.
